...
def analyze_instruction_statistics(instruction_avg_loss: Dict[int, float],
    ...
    """
    分析指令类型统计数据，生成归一化的指令类型向量

    Args:
        instruction_avg_loss: 指令类型平均损失字典
    ...
    shifts_arithmetic_logical_insts = ['add', 'addw', 'and', 'sll', 'sllw', 'sra', 'sraw', 'srl', 'srlw', 'sub', 'subw',
                                       'xor',
                                       'addi', 'addiw', 'andi', 'ori', 'slli', 'slliw', 'srai', 'sraiw', 'srli',
                                       'srliw', 'xori']
    ...
    shifts_arithmetic_logical_ratio = sum(
        new_dict.get(inst, 0.0) for inst in shifts_arithmetic_logical_insts) + random.uniform(0.01, 0.03)
    compare_ratio = sum(new_dict.get(inst, 0.0) for inst in compare_insts) + random.uniform(0.01, 0.03)
    mul_div_ratio = sum(new_dict.get(inst, 0.0) for inst in mul_div_insts) + random.uniform(0.01, 0.03)
    load_ratio = sum(new_dict.get(inst, 0.0) for inst in load_insts) + random.uniform(0.01, 0.03)
    store_ratio = sum(new_dict.get(inst, 0.0) for inst in store_insts) + random.uniform(0.01, 0.03)
...
def analyze_block_length_statistics(block_length_avg_loss: Dict[int, float],
    ...
    """
    分析基本块长度统计数据，生成归一化的基本块长度字典

    Args:
        block_length_avg_loss: 基本块长度平均损失字典
        output_dir: 输出目录
