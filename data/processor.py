...
class RISCVDataProcessor:
    ...
    # def process_raw_data(self, output_path: Optional[str] = None) -> List[Dict[str, Any]]:
    #     """
    #     处理原始数据并保存为中间JSON格式
    #
    #     Args:
    #         output_path: 输出JSON文件路径，默认使用config中的路径
    #
    #     Returns:
    #         处理后的数据列表
    #     """
    ...
    #     # 处理数据
    #     processed_data = []
    #     for item in tqdm(raw_data, desc="处理数据样本"):
    ...
    #
    #     print(f"已处理 {len(processed_data)} 条样本，保存到 {output_path}")
    ...
    #
    #         # 记录指令数量
    #         instruction_counts[i] = item["num_instructions"]
    #
    #         # 记录吞吐量
    #         Y[i] = item["throughput"]
    #
    ...
    # 在process_data_to_h5方法中添加原始指令文本存储
    ...
    #
    #         # 保存原始指令文本
    #         original_instructions.append(item["instructions"])
    #
    #     # 创建HDF5文件
    #     os.makedirs(os.path.dirname(output_path), exist_ok=True)
    #     with h5py.File(output_path, 'w') as f:
    ...
    #         # 存储原始指令文本，用于GNN模型
    #         dt = h5py.special_dtype(vlen=str)
    #         instr_text = np.array([json.dumps(instrs) for instrs in original_instructions], dtype=dt)
    #         f.create_dataset('instruction_text', data=instr_text)
    #
    #         # 存储元数据
    #         f.attrs['num_samples'] = num_samples
