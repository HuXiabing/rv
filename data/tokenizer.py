...
#     def __init__(self,
#                  vocab_size: int = 2000,
#                  max_instr_length: int = 20):
#         """
#         初始化RISC-V分词器
#
#         Args:
#             vocab_size: 词汇表大小的上限
#             max_instr_length: 每条指令的最大标记数
#         """
#         self.vocab_size = vocab_size
#         self.max_instr_length = max_instr_length
#         self.vocab = {"<PAD>": 0, "<UNK>": 1}
#         self.inverse_vocab = {0: "<PAD>", 1: "<UNK>"}
#         self.current_vocab_size = 2  # 初始包含PAD和UNK
#
...
#
#             # 提取操作数
#             # 处理形如 rd, rs1, rs2 或 rd, imm(rs1) 的格式
#             operands = re.split(r',\s*', operands)
#             for operand in operands:
#                 # 处理形如 imm(rs1) 的格式
#                 if '(' in operand and ')' in operand:
#                     imm, reg = operand.split('(', 1)
#                     reg = reg.rstrip(')')
#                     if imm:
#                         parts.append(imm)
#                     parts.append(reg)
...
#         return parts
#
#     def build_vocab_from_instructions(self, instructions_list: List[List[str]]) -> Dict[str, int]:
#         """
#         从指令列表构建词汇表
#
#         Args:
#             instructions_list: 指令列表的列表
#
#         Returns:
#             构建的词汇表字典
#         """
...
#
#         # 选择最常见的词构建词汇表（减去已有的PAD和UNK）
#         most_common = token_counts.most_common(self.vocab_size - 2)
#
#         # 构建词汇表
#         for token, _ in most_common:
#             if token not in self.vocab:
#                 self.vocab[token] = self.current_vocab_size
#                 self.inverse_vocab[self.current_vocab_size] = token
...
#
#         # 截断或填充到固定长度
#         if len(encoded) > self.max_instr_length:
#             encoded = encoded[:self.max_instr_length]
#         else:
#             encoded += [self.vocab["<PAD>"]] * (self.max_instr_length - len(encoded))
#
#         return encoded
#
#     def decode_instruction(self, token_ids: List[int]) -> str:
#         """
#         将token ID序列解码为指令字符串
#
#         Args:
...
#         """
#         # 过滤掉<PAD>标记
#         filtered_ids = [tid for tid in token_ids if tid != self.vocab["<PAD>"]]
#         tokens = [self.inverse_vocab.get(tid, "<UNK>") for tid in filtered_ids]
#
#         # 简单拼接（注意这不是完全可逆的，仅用于调试）
#         return " ".join(tokens)
#
...
#         self.inverse_vocab = {int(k): v for k, v in vocab_data["inverse_vocab"].items()}
#         self.current_vocab_size = vocab_data["vocab_size"]
#         self.max_instr_length = vocab_data.get("max_instr_length", self.max_instr_length)
